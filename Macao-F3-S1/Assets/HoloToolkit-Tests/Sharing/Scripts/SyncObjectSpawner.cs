...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class SyncObjectSpawner : MonoBehaviour
    {
        ...
        {
            Vector3 position = Camera.main.transform.position + Vector3.forward * 2;//Random.onUnitSphere * 2;
            Quaternion rotation = Camera.main.transform.rotation;//Random.rotation;
        ...
        public void SpawnHeadSyncObject()
        {
            Vector3 position = Camera.main.transform.position;
            Quaternion rotation = Camera.main.transform.rotation;

            var spawnedObject = new SyncSpawnedHeadObject();

            spawnManager.Spawn(spawnedObject, position, rotation, spawnParentTransform.gameObject, "SpawnedObject", false);
        }

        public void SpawnCarBasePointSyncObject()
        {
            GameObject gm = GameObject.Find("F1Collection_Pivot").gameObject;
            Vector3 position = gm.transform.position;
            Quaternion rotation = gm.transform.rotation;

            var spawnedObject = new SyncSpawnedCarBasePoint();
